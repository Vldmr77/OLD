...
репозитория сигналов и отображает последние записи, а также агрегированную статистику. Если в конфигурации
`dashboard.auto_start` оставлен включённым (значение по умолчанию), оркестратор автоматически запускает UI в
фоновом потоке на `dashboard.host:dashboard.port`. Путь до файла с сигналами задаётся параметром
`dashboard.repository_path` и по умолчанию совпадает с `storage.base_path/signals.db`.
