...
Если нужно протестировать систему без реальных ключей, выставите `datafeed.allow_tokenless: true`.
В этом режиме оркестратор запускает `OfflineMarketDataStream`, который повторно проигрывает локальный
датасет или генерирует синтетический стакан. Пока система работает в тестовом или форвард-режиме,
исполнение заявок остаётся «бумажным», поэтому запуск возможен даже без доступа к API брокера.
...
Эти параметры передаются и в поток маркет-даты, и в клиент исполнения, поэтому достаточно
